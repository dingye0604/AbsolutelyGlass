...
A warm, glass-panelled Obsidian theme. CLaudeApple keeps the layout, components, and motion of [ClaudeBaseline](https://github.com/dingye0604/ClaudeBaseline) and adds a semi-transparent frosted layer — and on Windows 11, an optional companion plugin lets the real desktop show through.

| Light | Dark |
| :---: | :---: |
| [![CLaudeApple in light mode](./screenshot-light.jpg)](./Fig1.jpg) | [![CLaudeApple in dark mode](./screenshot-dark.jpg)](./Fig2.jpg) |

*Click a screenshot to view it full size.*

## What you get

CLaudeApple is the third layer in a chain:

```
Baseline            community theme by aaaa-alexis (MIT)
  └─ ClaudeBaseline baseline layout, warm Claude-inspired palette and typography
       └─ CLaudeApple glass panels, ambient tint, translucent window
```

Everything below the glass is ClaudeBaseline. CLaudeApple only appends a material layer on top — it does not fork or replace the base styling.
...
- **Theme + companion plugin.** On Windows 11 22H2 or newer, the optional [CLaudeApple Glass](https://github.com/dingye0604/claudeapple-glass) plugin asks Electron for a native Acrylic backdrop, so your actual desktop shows through the window.
...
2. Search for **CLaudeApple**
3. **Install and use**

### Manually

1. Download `manifest.json` and `theme.css` from the [latest release](https://github.com/dingye0604/CLaudeApple/releases/latest)
2. Create a folder named `CLaudeApple` inside `<your-vault>/.obsidian/themes/`
3. Put both files in that folder
4. Reload Obsidian, then pick **CLaudeApple** in **Settings → Appearance → Themes**

### Optional: native Acrylic on Windows 11

Install the [CLaudeApple Glass](https://github.com/dingye0604/claudeapple-glass) plugin and enable it. Nothing else is required — the plugin finds the theme on its own.

The plugin manages the main window only. Do not run it alongside another Mica/Acrylic window plugin.

## Settings

With [Style Settings](https://github.com/mgmeyers/obsidian-style-settings) installed, **CLaudeApple · Glass material** exposes:
...
Everything else in the Style Settings panel comes from ClaudeBaseline and still works. CLaudeApple's material overrides take priority over some of Baseline's background and border options.
...
**The desktop doesn't show through.** The plugin is not active. Check that it is enabled, that you are on Windows 11 22H2 or newer, and that Settings → Accessibility → Visual effects → Transparency effects is on. Run **CLaudeApple Glass: Reapply Acrylic backdrop** from the command palette to retry.
...
**Switching back.** Choose ClaudeBaseline (or any other theme) — the plugin notices the theme is gone and restores the window. Disabling the plugin also restores the original window background color and removes its CSS marker. It does not touch any Windows setting.

## Credits

CLaudeApple is a derivative work and most of what you see is other people's work.
...
- **[ClaudeBaseline](https://github.com/dingye0604/ClaudeBaseline)** — the direct base of this theme. Its palette and typography are inherited unchanged.
...
[MIT](./LICENSE) © the CLaudeApple authors, incorporating ClaudeBaseline and Baseline © 2025 aaaa​alexis.

Bundled and referenced fonts are licensed separately under the SIL Open Font License 1.1 — see [Credits](#credits).

## For developers

`theme.css` is a build artifact: `ClaudeBaseline/theme.css` followed by `glass.css`. Edit `glass.css`, then rebuild. See [DEVELOPMENT.md](./DEVELOPMENT.md), and [VALIDATION.md](./VALIDATION.md) for what has and has not been verified.
